...
def _build_lmdb_dataset(
    ...
    """
    # Changes were made here to decouple the dataset path from the lmdb path
    if opt is not None:
        image_net_root = str(root)
        root = str(opt.lmdb_dir)
    else:
        image_net_root = str(root)
        root = str(root)
    if root.endswith('/'):
        root = root[:-1]
    if image_net_root.endswith('/'):
        image_net_root = image_net_root[:-1]
...
def build_lmdb_dataset(opt, log, train, transform=None):
    ...
    dataset = _build_lmdb_dataset(fn, log, transform=transform, opt = opt)
...
def build_lmdb_dataset_val10k(opt, log, transform=None):
    # Adjusted Hardcoded paths to read the path which describes the dataset from the opt config
    ...
    # Also split the real dataset path and the lmdb datapath
    fn_10k_path = opt.lmdb_dir / "val_faster_imagefolder_10k_fn.txt" 
    label_10k_path = opt.lmdb_dir / "val_faster_imagefolder_10k_label.txt"
    fn_10k = readlines(fn_10k_path)
    label_10k = readlines(label_10k_path)
    if transform is None: transform = build_test_transform(opt.image_size)
    dataset = _build_lmdb_dataset(opt.dataset_dir / 'val', log, transform=transform, opt=opt)
